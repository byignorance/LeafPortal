...
struct PortalProjectDropboxBrowserSheet: View {
    @ObservedObject var manager: PortalProjectArchiveManager
    let archive: ProjectArchiveSummary
    @Binding var isShowing: Bool

    @State private var selectedFolderID: String?
    @State private var currentListing: ProjectDropboxFolderListing?
    ...
    private var visibleFolders: [ProjectStorageFolder] {
        manager.dropboxVisibleFolders(for: archive)
    }
    ...
    private var currentUploadProgress: PortalProjectArchiveManager.UploadProgressState? {
        guard manager.uploadProgress?.provider == .dropbox else { return nil }
    ...
    private var groupedFolderItems: [ProjectDropboxItem] {
        currentListing?.items.filter(\.isFolder) ?? []
    }

    private var looseFileItems: [ProjectDropboxItem] {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("\(archive.projectName) Dropbox")
                        .font(.system(size: 26, weight: .black))
                    Text("프로젝트 허브 안에서 Dropbox 폴더별 파일 목록을 보고 업로드할 수 있습니다.")
                ...
                if let rootURL = archive.dropboxRootWebURL {
                    Link(destination: rootURL) {
                        Label("Dropbox에서 열기", systemImage: "arrow.up.right.square")
            ...
            HStack(alignment: .top, spacing: 16) {
                            ...
                            Button {
                                ...
                                HStack(spacing: 10) {
                                    Image(systemName: "shippingbox.fill")
                                        .foregroundStyle(Color(red: 0.04, green: 0.27, blue: 0.80))
                ...
                PortalCard(padding: 18) {
                    VStack(alignment: .leading, spacing: 14) {
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    HStack(spacing: 8) {
                                        Text(currentListing?.folderTitle ?? selectedFolder?.title ?? "Dropbox")
                                            .font(.system(size: 16, weight: .bold))
                                        if let selectedFolder,
                                           let count = folderItemCounts[selectedFolder.id] {
                                            Text("\(count)개")
                                                .font(.system(size: 12, weight: .bold))
                                                .foregroundStyle(Color.black.opacity(0.45))
                                        }
                                ...
                                if let selectedFolder {
                                    ...
                                    .buttonStyle(.plain)
                                    .disabled(manager.isLoadingDropboxFolderContents || isPerformingUpload || manager.isUploadingDropboxFile)
                                    .opacity((manager.isLoadingDropboxFolderContents || isPerformingUpload || manager.isUploadingDropboxFile) ? 0.5 : 1)
                                    .clickableCursor(enabled: !(manager.isLoadingDropboxFolderContents || isPerformingUpload || manager.isUploadingDropboxFile))
                                }

                                PortalSecondaryButton(
                                    title: isPerformingUpload || manager.isUploadingDropboxFile ? "업로드 중..." : "파일 업로드",
                                    disabled: selectedFolder == nil || isPerformingUpload || manager.isUploadingDropboxFile
                                ) {
                                    Task {
                        ...
                        if let currentUploadProgress, (isPerformingUpload || manager.isUploadingDropboxFile) {
                        ...
                        if manager.isLoadingDropboxFolderContents {
                            VStack(spacing: 10) {
                                ProgressView()
                                Text("폴더 목록을 불러오는 중입니다.")
        ...
        .overlay {
            if isPerformingUpload || manager.isUploadingDropboxFile {
                ZStack {
                    ...
                    VStack(spacing: 12) {
                        ProgressView(value: currentUploadProgress?.fractionCompleted ?? 0, total: 1)
                            .progressViewStyle(.linear)
                            .frame(width: 220)
                        Text(currentUploadProgress?.fileName ?? "Dropbox에 업로드 중입니다...")
                            .font(.system(size: 13, weight: .bold))
        ...
        .sheet(isPresented: $isShowingUploadMetadataSheet, onDismiss: clearPendingUpload) {
            PortalProjectStorageUploadMetadataSheet(
                ...
                folderTitle: selectedFolder?.title ?? "Dropbox",
                selectedFiles: selectedFileNames,
                keywordsPlaceholder: "편집본, 렌더, 소스",
                ...
                isSubmitting: isPerformingUpload || manager.isUploadingDropboxFile,
        ...
        .task {
            ...
            if let folder = selectedFolder {
                currentListing = manager.cachedDropboxFolderListing(archive: archive, folder: folder)
                await loadListing(for: folder)
            }
        }
    ...
    private var defaultFileIcon: String {
        "doc.fill"
    }

    @ViewBuilder
    private func dropboxItemRow(_ item: ProjectDropboxItem) -> some View {
    ...
    private func itemRowContent(_ item: ProjectDropboxItem) -> some View {
        HStack(spacing: 12) {
            Image(systemName: item.isFolder ? "shippingbox.fill" : fileIcon(for: item.name))
                .font(.system(size: 14, weight: .bold))
                .foregroundStyle(item.isFolder ? Color(red: 0.04, green: 0.27, blue: 0.80) : Color(red: 0.19, green: 0.20, blue: 0.24))
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 8) {
                    ...
                    if !item.isFolder, let sizeText = fileSizeText(for: item.sizeBytes) {
                        Text(sizeText)
                    }
    ...
    private func fileIcon(for fileName: String) -> String {
        ...
        case "mp4", "mov", "m4v", "avi", "mkv", "webm", "flv", "wmv", "mxf":
            return "play.rectangle.fill"
        case "png", "jpg", "jpeg", "gif", "webp", "heic", "heif":
            return "photo.on.rectangle.angled"
        case "zip", "rar", "7z", "tar", "gz":
            return "archivebox.fill"
        case "psd", "ai", "prproj", "aep":
    ...
    private func loadListing(for folder: ProjectStorageFolder) async {
        currentListing = await manager.loadDropboxFolderListing(
            archive: archive,
            folder: folder
        )
    ...
    private func confirmPendingUpload() async {
        ...
        for (index, fileURL) in pendingUploadFileURLs.enumerated() {
            ...
            let result = await manager.uploadFileToDropbox(
                archive: archive,
                folder: folder,
                fileURL: fileURL,
                workGroupName: uploadWorkGroupName,
    ...
    private func refreshQueueRowsPreview() {
        guard !(isPerformingUpload || manager.isUploadingDropboxFile) else { return }
    ...
    private func seedFolderCountsFromCache() {
        for folder in visibleFolders {
            if let count = manager.cachedDropboxFolderItemCount(archive: archive, folder: folder) {
    ...
    private func preloadFolderCounts() async {
        ...
        for folder in visibleFolders {
            ...
            if let count = await manager.loadDropboxFolderItemCount(
                archive: archive,
                folder: folder
            ) {
                folderItemCounts[folder.id] = count
    ...
    private func folderWebURL(for folder: ProjectStorageFolder) -> URL? {
        ...
        if selectedFolderID == folder.id, let currentListing {
            folderPath = currentListing.folderPath
        } else {
            folderPath = manager.cachedDropboxFolderListing(archive: archive, folder: folder)?.folderPath
        }

        guard let folderPath, !folderPath.isEmpty else {
            return archive.dropboxRootWebURL
        }

        var components = URLComponents(string: "https://www.dropbox.com")
        components?.percentEncodedPath = "/home" + folderPath.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)!.replacingOccurrences(of: "%2F", with: "/")
